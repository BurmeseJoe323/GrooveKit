...
TrackEditView::TrackEditView (AppEngine& engine)
{
    ...
    appEngine->onEditLoaded = [this] {
        ...
        if (bpmEditField)
            bpmEditField->setValue (appEngine->getBpm(), juce::dontSendNotification);
...
void TrackEditView::setupButtons ()
{
    ...
    bpmEditField = std::make_unique<BPMEditComponent>();
    addAndMakeVisible (bpmEditField.get());
    bpmEditField->setValue (appEngine->getBpm(), juce::dontSendNotification);
    bpmEditField->onValueChange = [this](double newBpm) {
...
// ============================================================================
// BPMEditComponent Implementation
// ============================================================================

TrackEditView::BPMEditComponent::BPMEditComponent()
{
    setWantsKeyboardFocus (true);

    addChildComponent (textEditor);
    textEditor.setEditable (true);
    textEditor.setJustificationType (juce::Justification::centred);
    textEditor.setColour (juce::Label::textColourId, juce::Colours::lightgrey);
    textEditor.setColour (juce::Label::backgroundColourId, juce::Colours::darkgrey.darker());
    textEditor.setColour (juce::Label::outlineColourId, juce::Colours::lightgrey.brighter (0.5f));
    ...
    textEditor.onEditorHide = [this] {
        stopEditing (true);
    };
}

void TrackEditView::BPMEditComponent::paint (juce::Graphics& g)
{
    auto bounds = getLocalBounds();

    // Background color - brighter when focused, normal when not
    if (isFocused || isDragging)
        g.setColour (juce::Colours::darkgrey.darker().brighter (0.2f));
    else
        g.setColour (juce::Colours::darkgrey.darker());

    g.fillRect (bounds);

    // Outline - brighter when focused/dragging
    if (isFocused || isDragging)
        g.setColour (juce::Colours::lightgrey.brighter (0.7f));
    else
        g.setColour (juce::Colours::lightgrey.brighter (0.5f));
    ...
    if (!isEditing)
    {
        ...
        g.setFont (14.0f);
        g.drawText (juce::String (currentValue, 2), bounds, juce::Justification::centred);
    }
}

void TrackEditView::BPMEditComponent::resized()
{
    textEditor.setBounds (getLocalBounds());
}

void TrackEditView::BPMEditComponent::mouseDown (const juce::MouseEvent& e)
{
...
void TrackEditView::BPMEditComponent::mouseDrag (const juce::MouseEvent& e)
{
    // If we haven't started dragging yet and moved enough, start dragging
    if (!isDragging && !isEditing)
    {
        const int deltaY = std::abs (e.getPosition().y - dragStartPosition.y);
        if (deltaY > 2) // Small threshold to distinguish drag from click
        {
            isDragging = true;
...
void TrackEditView::BPMEditComponent::mouseUp (const juce::MouseEvent& e)
{
    if (isDragging)
    {
        // Restore cursor at the original click position
        isDragging = false;
        setMouseCursor (juce::MouseCursor::NormalCursor);

        // Reposition mouse to original click position
        e.source.enableUnboundedMouseMovement (false);
        auto screenPos = localPointToGlobal (dragStartPosition);
    ...
    {
        // This was a click without drag - start editing
        startEditing();
    }
}

void TrackEditView::BPMEditComponent::focusGained (juce::Component::FocusChangeType)
{
    isFocused = true;
    repaint();
}

void TrackEditView::BPMEditComponent::focusLost (juce::Component::FocusChangeType)
{
...
void TrackEditView::BPMEditComponent::setValue (double newValue, juce::NotificationType notification)
{
    newValue = ValidationUtils::constrainAndRoundBpm (newValue, minValue, maxValue);
...
void TrackEditView::BPMEditComponent::startEditing()
{
    if (isEditing)
        return;
...
void TrackEditView::BPMEditComponent::stopEditing (bool applyChanges)
{
    if (!isEditing)
        return;
...
void TrackEditView::BPMEditComponent::updateValueFromDrag (int deltaY)
{
    // Scale: 1 pixel = 0.5 BPM change
    const double change = deltaY * 0.5;
    setValue (dragStartValue + change, juce::sendNotification);
}

void TrackEditView::BPMEditComponent::updateValueFromText()
{
    const std::string text = textEditor.getText().toStdString();
    ...
    const double newValue = std::stod (text);
    setValue (newValue, juce::sendNotification);

    // Update the text editor to show the constrained/rounded value
    textEditor.setText (juce::String (currentValue, 2), juce::dontSendNotification);
}
